...
// DEMO ONLY. Credentials checked in the browser are not security.
// Before launch, swap signIn() for a real auth provider (e.g. Supabase Auth email + password).
...
function signIn(email: string, password: string): Promise<Session | null> {
  return new Promise(res => window.setTimeout(() => res(email.trim().toLowerCase() === DEMO_ACCOUNT.email && password === DEMO_ACCOUNT.password ? {
  ...
  return <div className="admin grid min-h-[100dvh] w-full bg-canvas text-fg lg:grid-cols-[minmax(420px,1fr)_1.1fr]">
      ...
      <main className="flex items-center justify-center px-5 py-12">
        <div className="w-full max-w-[380px]">
          ...
          <div className="mt-6 rounded-lg border border-dashed border-stroke-strong px-3.5 py-3 text-[13px] text-fg-muted">
