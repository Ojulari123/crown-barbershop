...
import { DEMO_MODE, resetDemo, useCrown } from '@/lib/store';
...
export function SettingsAdmin({
  ...
  function exportCsv() {
    ...
    const esc = (v: string) => `"${(/^[=+\-@\t\r]/.test(v) ? `'${v}` : v).replace(/"/g, '""')}"`; // no spreadsheet formulas from the public form
  ...
  return <>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Panel title="Records" bodyClass="grid gap-3 p-4">
          ...
          {DEMO_MODE && <div className="mt-2 border-t border-stroke pt-3">
