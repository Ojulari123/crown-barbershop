...
const S: Record<Size, string> = {
  sm: 'h-8 px-2.5 text-[13px] gap-1.5 pointer-coarse:h-11',
...
export function IconButton({
  ...
  return <button type="button" aria-label={label} title={label} {...rest} className={cx(base, V[variant], size === 'sm' ? 'h-8 w-8 pointer-coarse:h-11 pointer-coarse:w-11' : 'h-9 w-9 pointer-coarse:h-11 pointer-coarse:w-11', className)}>
      {icon}
    </button>;
}
...
export const inputCls = (err?: boolean, extra?: string) => cx('h-9 w-full rounded-lg bg-panel px-3 text-[14px] text-fg ring-1 ring-inset placeholder:text-fg-subtle focus:outline-none focus:ring-2 pointer-coarse:h-11 max-sm:text-[16px]', err ? 'ring-red-500 focus:ring-red-500' : 'ring-stroke-strong focus:ring-accent', extra);
...
export function Segmented<T extends string>({
  ...
  return <div role="tablist" aria-label={label} className="inline-flex rounded-lg bg-sunken p-0.5 ring-1 ring-inset ring-stroke">
      ...
      return <button key={o.id} role="tab" type="button" aria-selected={active} onClick={() => onChange(o.id)} className={cx('relative inline-flex h-8 items-center gap-1.5 rounded-md px-3 text-[13px] font-medium pointer-coarse:h-11', active ? 'text-fg' : 'text-fg-muted hover:text-fg')}>
